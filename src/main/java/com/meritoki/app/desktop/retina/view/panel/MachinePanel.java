...
public class MachinePanel extends JPanel implements MouseListener, KeyListener, Runnable, HierarchyListener {
    ...
    public void run() {
        ...
        while (this.meritoki.retina.loop) {
            this.init();
            try {
                Thread.sleep(100);
            } catch (Exception e) {
...
//        System.out.println("paint("+String.valueOf(graphics != null)+")");
    ...
    /**
     * Here is mouse is pressed, this is the start of many operations. Here we
     * know what page is selected, if the event lies in or outside of a
     * shape,etc. Commands we need to support include Add, Select, Move, Resize,
     * and Remove Shape the commands that we need to support require checking
     * the current page for a contains or intersect with respect to Shapes. if
     * contains or intersect are false, we have an Add. Every other case is a
     * Select followed by a Move, Resize, or Remove Looking for a general
     * architecture to capture these possibilities that is maintainable and
     * supports the Do/Undo/Redo system An undo/redo also remembers the Page
     * where something happened, taking us to the page where something is undone
     * if necessary. Here we know that we are choosing an existing Shape This
     * can be used for Select, Remove, Move, If point pressed and point released
     * are equal we have a select, once we have a select we can Remove. if
     * pressed point does not equal released, we have a move. Here we know we
     * are choosing an edge to resize, and we know what edge or corner is
     * selected based on the selection integer value, all we need now is the
     * release point.
     */
    @Override
    ...
    /**
     * Here a mouse is released, telling us where an Add, Move, Resize, and
     * Remove
     */
    @Override
        ...
        if (ke.isControlDown()) {
            switch (ke.getKeyCode()) {
                case KeyEvent.VK_EQUALS: {
                    this.meritoki.retina.deltaDistance(distanceFactor);
                    this.mainFrame.init();
                    break;
                }
                ...
                case KeyEvent.VK_S: {
                    if (this.thread != null && this.thread.isAlive()) {
                        this.meritoki.retina.loop = false;
    ...
    public void start() {
    	this.meritoki.retina.setDistance(0);
        this.meritoki.retina.loop = true;
